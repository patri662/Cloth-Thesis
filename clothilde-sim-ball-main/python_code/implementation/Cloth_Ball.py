...
class Cloth:
    ...
    # Method to visualize the simulation, including the case where ball initialized
    # if ball initialized, this method will also make the movie for the ball
    # better to keep all in same method so they sync well
    def makeMovie(self, speed = 1, repeat = True, smooth = 0):

        if self.polyscoped is False:
            self.preparePolyscope()
        # for the ball
        if hasattr(self, 'ball') and not self.ball.polyscoped:
            self.ball.preparePolyscope()
        ...
        def goThroughHistory():
            # Update Polyscope visualization for cloth
            ...
            # Update Polyscope visualization for ball
            if hasattr(self, 'ball'):
                ball_pos = self.ball.history_pos[self.ps_frame]
                ps.get_point_cloud(self.ball.label).update_point_positions(ball_pos[np.newaxis, :])
            ...
            if self.ps_frame >= len(self.history_pos):
                if repeat:
                    ...
                    ps.get_surface_mesh(self.label).update_vertex_positions(phi_all)
                    ps.get_point_cloud(self.label).update_point_positions(phi_mat)
                    # check if ball object exists
                    if hasattr(self, 'ball'):
                        ps.get_point_cloud(self.ball.label).update_point_positions(
                        self.ball.history_pos[-1][np.newaxis, :])
    ...
    def setSimulatorParameters(self, dt = 1/60, tol = 0.0075, sub_steps = 10,
        ...
        # make dt and sub_steps accessible to the Ball class for simulation method
        # hasattr returns True if the specified object has the specified attribute/method, otherwise False
        # if never call cloth.addBall, self.ball doesn't exist and False so Ball code not touched
        if hasattr(self, 'ball'): 
    ...
    def selfCollisions(self,phi,n_iter,max_iters=50):    
        if n_iter == 0: # only called once at start, so not update within substep
            #precompute objects for selfcollisions
            self.prepareCollisions(phi)      
        #check for possible selfcollisions -> update within substeps!
    ...
    def updateClosePairs(self,phi_mat):
        ...
        # for ball collisions update sync with autocollisions
        # Ball moves independently of cloth — always re-check,
        if hasattr(self, 'ball'):
    ...
    # DEFINITION OF THE BALL OBJECT INSIDE CLOTH CLASS -> so we have cloth.ball.X
    class Ball: # give _ball name to variables to avoid identical definitions issues
        def __init__(self, position, rad, mass, friction, 
                     velocity = None, name = 'ball'): # name for polyscope
            
            #positions and velocities of ball
            self.position = np.array(position, dtype=float) #current position of ball (3,)
            self.velocity = np.zeros(3) if velocity is None else np.array(velocity, dtype=float)
            self.history_pos = [self.position] #history of the vertices of the ball
            self.history_vel = [self.velocity] #history of the velocities of the ball
            
            # dimension of object
            self.rad = rad # given rad of the ball
            ...
            # default physical parameters of the object
            self.mass = mass # given weight of object
            self.mu_b = friction #friction of ball
            ...
            # for collisions with cloth
            # got from Cloth, method to query cloth vertices against ball -> computeClosePairsBall()
            self.near_cloth = np.array([], dtype=int)  # indices of close cloth vertices
            self.dists_cloth = np.array([], dtype=float)  # distances to those vertices

        # restart ball position 
        def restart(self):
            self.position = self.history_pos[0]
            self.velocity = self.history_vel[0]
            self.history_pos = [self.position] 
            self.history_vel = [self.velocity] 
            self.total_iters = 0
            self.warning = False

        #save final positions and velocities
        def save(self):
            """Call once per simulate() frame to record state."""
            self.history_pos.append(self.position)
            self.history_vel.append(self.velocity)

        def __repr__(self):
            return f"SolidBall(rad={self.rad}, mass={self.mass}, p={self.position})"

        # Methods to plot the ball, same ones as the cloth method 
        # but without full initialization, as cloth will already do it
        # only register the ball as a point cloud with rad
        def preparePolyscope(self):
            self.polyscoped = True
            ps.register_point_cloud(self.label, self.position[np.newaxis, :],
                                radius=self.rad)
        ...
        def plotMesh(self):    
            if self.polyscoped is False:
                self.preparePolyscope()
            """Plot the current mesh"""
            ps.get_point_cloud(self.label).update_point_positions(self.position[np.newaxis, :])
            ps.show()

        # ball's motion is free-fall, mirroring unconstrainedStep from Cloth class
        def unconstrainedStep(self, dt, g=9.8):
        ...
        def simulate(self, g=9.8):
            self.unconstrainedStep(self.dt,g)  # dt comes from Cloth class


    # method inside Cloth to add the ball by calling cloth.addBall()
    def addBall(self, position, rad, mass = 1.0, friction = 0.3, velocity = None):
        return self.Ball(position, rad, mass, friction, velocity)
    
    # method to query cloth vertices against ball -> called in Cloth.selfCollisions() in Cloth.simulate(), not in Ball.simulate()!!
    ...
    def computeClosePairsBall(self, phi_mat):
        # vector from each cloth vertex to ball center
        diff = phi_mat - self.ball.position[np.newaxis, :]  # (n_verts, 3)
    ...
    def ballCollisions(self, phi, gamma0, n_iter, max_iters=50):
        ...
        # Always recompute with current phi, ball position may have shifted from previous iter
        self.computeClosePairsBall(phi_mat)

        if self.ball.near_cloth.shape[0] == 0:
            return phi
        
        idx = self.ball.near_cloth
        
        # vectors from ball center to each close cloth vertex
        diff = phi_mat[idx] - self.ball.position[np.newaxis, :]  # (k,3) pointing away from ball
        dists = np.sqrt(np.einsum('ij,ij->i', diff, diff)) + 1e-12  # (k,)
    ...
    def simulate(self, u, control):
        ...
        for s in range(self.sub_steps):
            ...
            # check if ball class initialized and get current position
            if hasattr(self, 'ball'):
            ...
            # check if ball class initialized to simulate its physics along cloth
            if hasattr(self, 'ball'):
                self.ball.simulate()
            ...
            while (error_str > self.tol or error_shr > self.tol) and n_iter < 100: 
                ...
                # ball-cloth collisions -> same iterative treatment as cloth self-collisions
                if hasattr(self, 'ball'):
                    phi = self.ballCollisions(phi, gamma0, n_iter)
        ...
        # save ball state once per frame (after all substeps)
        if hasattr(self, 'ball'):
            self.ball.history_pos.append(self.ball.position.copy())
            self.ball.history_vel.append(self.ball.velocity.copy())
