...
class Cloth:
    ...
    def setSimulatorParameters(self, dt = 1/60, tol = 0.0075, sub_steps = 10,
                               rho = 0.1, delta = 0.1, alpha = 0.2, k_aero = 0.0,
    ...
    def TrapezoidalRule(self):
        # velocity-dependent delta
        node_speeds = self.computeNorm(self.velocities)
        mean_vel = np.mean(node_speeds)
        delta_eff = self.delta + self.k_aero * mean_vel
        half_dt2_delta_eff_Fg = 0.5 * (self.dt**2) * delta_eff * self.g * self.Fg # new calculated Fg with new delta
        q = half_dt2_delta_eff_Fg + (self.dt_rho_M * self.velocities) + (self.E_aux @ self.positions)
